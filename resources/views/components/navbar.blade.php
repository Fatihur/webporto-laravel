            ...
            <div class="hidden md:flex items-center space-x-10">
                @foreach($navLinks as $link)
                    @if($link['name'] === 'Projects')
                        <div class="relative group" id="projects-menu">
                            <button
                                ...
                                class="flex items-center gap-1 text-sm font-semibold hover:text-mint transition-colors {{ str_starts_with($currentRoute, 'projects') ? 'text-mint' : 'text-zinc-600 dark:text-zinc-300' }}"
                            >
                                {{ $link['name'] }}
                    ...
                    @else
                        <a
                            href="{{ $link['path'] }}"
                            class="text-sm font-semibold hover:text-mint transition-colors {{ $currentRoute === $link['route'] ? 'text-mint' : 'text-zinc-600 dark:text-zinc-300' }}"
                        >
                            {{ $link['name'] }}
                        </a>
                    @endif
...
>
    <div class="flex items-center justify-between p-6 border-b border-zinc-50 dark:border-zinc-900">
        ...
        <button
            ...
            class="p-2 rounded-full bg-zinc-50 dark:bg-zinc-900 text-zinc-400"
        >
                ...
                @foreach($navLinks as $link)
                    @if($link['name'] !== 'Projects')
                        <a
                            href="{{ $link['path'] }}"
                            onclick="closeMobileMenu()"
                            class="text-3xl font-bold tracking-tighter hover:text-mint transition-colors block {{ $currentRoute === $link['route'] ? 'text-mint' : 'text-zinc-950 dark:text-white' }}"
                        >
                            {{ $link['name'] }}
                        </a>
                    @endif
